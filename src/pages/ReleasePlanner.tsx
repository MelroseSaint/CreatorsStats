  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-[#8A9099] mt-2">Work backwards from your drop date to hit every deadline.</p>
      </div>
          ...
          <div className="bg-[#14161C] p-6 rounded-xl border border-[#1F222A]">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-sm text-[#8A9099] font-medium">Type</label>
                <select 
                  ...
                  <div key={project.id} className="bg-[#14161C] border border-[#1F222A] rounded-xl overflow-hidden">
                    <div className="p-4 bg-[#0B0C10] border-b border-[#1F222A] flex justify-between items-center">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                           <h3 className="font-bold text-[#F3F4F6] text-lg">{project.name}</h3>
                           <p className="text-xs text-[#5F646C] uppercase tracking-wider">{project.type} • Release: {project.releaseDate}</p>
                         </div>
                      </div>
                      <div className="flex items-center gap-4">
                         <div className="text-right">
                           <span className="block text-2xl font-bold text-[#F3F4F6]">{daysLeft}</span>
                           <span className="text-xs text-[#5F646C]">DAYS LEFT</span>
                         </div>
                         <button onClick={() => deleteProject(project.id)} className="p-2 text-[#5F646C] hover:text-red-500 transition-colors">
                           <Trash2 size={18} />
                         </button>
                      </div>
                    </div>
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4">
                      {deadlines.map((task, idx) => (
                        <div key={idx} className="bg-[#0B0C10] p-3 rounded border border-[#1F222A]">
                          <div className="flex items-center gap-2 mb-1">
                            <CalendarIcon size={12} className="text-[#5F646C]" />
                            <span className="text-xs text-[#5F646C]">{format(task.date, 'MMM d')}</span>
                          </div>
                          <p className="text-sm font-medium text-[#8A9099]">{task.label}</p>
                        </div>
                      ))}
                    </div>
                  </div>
